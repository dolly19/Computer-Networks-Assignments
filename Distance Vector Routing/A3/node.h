...
public:
	...
	void addInterface(string ip, string connip, Node *nextHop)
	{
		...
		interfaces.push_back(make_pair(eth, nextHop));
	}
	...
	void updateTblEntry(string breakIP)
	{
		for (int i = 0; i < mytbl.tbl.size(); i++)
		{
			if (mytbl.tbl[i].dstip == breakIP)
				mytbl.tbl[i].cost = 16;
			
		}
	}
	vector<string> AllInterfaces()
	{
		vector<string> res;
		for (int i = 0; i < interfaces.size(); i++)
		{
			res.push_back(interfaces[i].first.getip());
		}
		return res;
	}
	...
	void sendMsg()
	{
		...
		for (int i = 0; i < interfaces.size(); ++i)
		{
			if (count == 0)
			{
				...
				{
					RoutingEntry tempEntry;
					tempEntry = mytbl.tbl[j];
					
					tempEntry.share_cost = tempEntry.cost;
					if (mytbl.tbl[j].nexthop == interfaces[i].first.getConnectedIp())
						tempEntry.share_cost = 16;
				

					ntbl.tbl.push_back(tempEntry);
				}
				count++;
			}
