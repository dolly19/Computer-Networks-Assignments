  	...
  	for(int i=0;i<num;i++){
		...
		string normDstip = mytbl.tbl[i].dstip;
  		if(normDstip == tempDstip)
  		{
			...
			int shareCost = (*(*msg).mytbl).tbl[j].share_cost+1;

			if(isMyInterface(mytbl.tbl[i].nexthop))
				break;
				
			if( mytbl.tbl[i].nexthop == (*msg).from){
  			...
  			else if(shareCost < origCost )
  			{
  				
				mytbl.tbl[i].nexthop=(*msg).from;
				mytbl.tbl[i].cost=min(16,shareCost);
				mytbl.tbl[i].share_cost = mytbl.tbl[i].cost;
